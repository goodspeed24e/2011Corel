...
/* 需要用到随机密码标识符should use cryptographically random identifier is RECOMMENDED.... */
...
/* 在会话之外准备一个最小请求prepare a minimal request (outside of a dialog) with required headers */
/* 
   方法名是请求的类型("INVITE", "REGISTER"...)method_name is the type of request. ("INVITE", "REGISTER"...)
   to is the remote target URI
   使用TCP或者UDP传输都可以（当前使用的是UDP）transport is either "TCP" or "UDP" (by now, only UDP is implemented!)
*/
int
				 ...
				 char *to, char *transport)
{
  ...
  */
  static int register_cseq_number = 1; /* 注册总是从1开始always start registration with 1 */
  int i;
  char *max_fwd;
  char *proxy_require;
  sip_t *request;
  ...
  /* prepare the request-line准备邀请线程 */
  ...
  if (0==strcmp("REGISTER", method_name))
    ...
    {
      /* 除注册（REGISTER）状态任何状态都适用in any cases except REGISTER: */
	  ...
	  if (lr_param!=NULL) /* to is the remote target URI in this case! */
	    {
	      ...
	      /* "[request] MUST includes a Route header field containing
	       the route set values in order." [request]必须包括一个路由器头，其中包含有路由器设置值*/
	    ...
	    /* if the first URI of route set does not contain "lr", the rquri
		   ...
		   如果路由器设置的第一个URI不包含"lr"，则邀请被设为此URI*/
	    {
