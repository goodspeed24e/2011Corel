...
#ifdef _UNICODE
    wcsncpy_s( wstrDestination, cchDestChar, tstrSource, cchDestChar );
...
#ifdef _UNICODE
    wcsncpy_s( tstrDestination, cchDestChar, wstrSource, cchDestChar );
...
VOID DXUtil_LaunchReadme( HWND hWnd )
{
    ...
    if( !bFound )
    {
        ...
        _tcscpy_s( strReadmePath, sizeof(strReadmePath), strExePath );
...
VOID DXUtil_Trace( TCHAR* strMsg, ... )
{
    ...
    _vsntprintf_s( strBuffer, sizeof(strBuffer), _TRUNCATE, strMsg, args );
    va_end(args);
...
{
    int nResult = _sntprintf_s( strDest, cchDestChar, _TRUNCATE, TEXT("{%0.8X-%0.4X-%0.4X-%0.2X%0.2X-%0.2X%0.2X%0.2X%0.2X%0.2X%0.2X}"),
