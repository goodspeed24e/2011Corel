...
HRESULT CVideoSourceMSEvr::NonDelegatingQueryInterface(REFIID riid, void ** ppv)
{
  ...
  //  }
    ...
    else if (riid == __uuidof(IDisplayVideoSourceFRCControl))
    {
        hr = GetInterface((IDisplayVideoSourceFRCControl *) this, ppv);
    }
    else
    {
...
HRESULT CVideoSourceMSEvr::OnClockSetRate(MFTIME hnsSystemTime, float fRate)
{
    ...
    {
        CancelFrameStep();
		// If frame-step queue not empty, return all samples back to sample pool,
		while (!m_FrameStep.samples.IsEmpty())
		{
			CComPtr<IMFSample> pSample;
			m_FrameStep.samples.RemoveFront(&pSample);
			OnSampleFree(pSample);
		}
    }
...
{
    float fAspectRatio = m_fNativeAspectRatio;

    CD3D9VideoMixerModel *pModel = 0;
	CResourceManager::GetInstance()->GetInterface(DISPSVR_RESOURCE_VIDEOMIXERMODEL, __uuidof(IUnknown), (void **)&pModel);
    if (pModel)
    {
        pModel->Release();
        CAutoLock lockModel(pModel);
        fAspectRatio = pModel->GetPlane(PLANE_MAINVIDEO).GetAspectRatio();
    }

    if (pszMax)
    {
        if (fAspectRatio)
            pszMax->cx = (INT)ceil((float)m_lImageHeight * fAspectRatio);
        else
...
HRESULT CVideoSourceMSEvr::GetCurrentImage(BITMAPINFOHEADER* pBih, BYTE** pDib, DWORD* pcbDib, LONGLONG* pTimeStamp)
{
	...
	CComPtr<IMFSample> pSample;
    hr = m_scheduler.PeekPresentSample(&pSample);
...
//	m_pMediaType.Release();
	...
	m_dwFrameRate1000 = m_lImageWidth = m_lImageHeight = 0L;
}
//-----------------------------------------------------------------------------
// ConfigureMixer
//
    ...
    if (SUCCEEDED(hr))
    {
        ...
        while (hr != MF_E_NO_MORE_TYPES)
        {
            ...
            if (SUCCEEDED(hr) && SUCCEEDED(pType->GetGUID(MF_MT_SUBTYPE, &guidOutSubtype)))
            {
				...
				{
					if ((bIsSORTPresenter && PLANE_FORMAT_NV12 == dwOutSubFourCC) || (PCI_VENDOR_ID_INTEL == dwVendorID))
					{
		...
		if (iConnectMTIdx >= 0)
		{
			...
			if (SUCCEEDED(hr))
			{
                // let the default mixer outputs nominal range to full 0 - 255 if YUV -> RGB is done in EVR.
                hr = pType->SetUINT32(MF_MT_VIDEO_NOMINAL_RANGE,  DXVA2_NominalRange_Normal);
...
//		hr = m_SamplePool.AllocateSamples(pDevice, uWidth, uHeight, sampleQueue);
	...
	m_dwFrameRate1000 = Numerator / Denominator * 1000;
...
HRESULT CVideoSourceMSEvr::ProcessOutput()
{
    ...
    else
    {
        ...
        // Set up notification for when the sample is released.
			CHECK_HR_GOTO(hr = TrackSample(pSample));
		}

        // Schedule the sample.
        if ((m_FrameStep.state == FRAMESTEP_NONE) || bRepaint)
        {
            CHECK_HR_GOTO(hr = DeliverSample(pSample, bRepaint));
        }
        else
        {
...
HRESULT CVideoSourceMSEvr::DeliverFrameStepSample(IMFSample *pSample)
{
    ...
    else
    {
        ...
        {
			// Bug#99366
			// Because the DeliverSample() will trigger a callback from CEvrPresentScheduler::UpdatePresentSample()
			// Change the m_FrameStep.state and m_FrameStep.pSampleNoRef first to avoid asynchronous issue.
...
HRESULT CVideoSourceMSEvr::OnSampleFree(IMFSample *pSample)
{
    ...
    {
#ifdef _DEBUG
		// double check if the sample is not being used by the video mixer.
		CComPtr<IMFMediaBuffer> pBuffer;
...
STDMETHODIMP CVideoSourceMSEvr::GetSampleProperty(SampleProperty *pProp)
{
    ...
    if (SUCCEEDED(hr) && pSample)
    {
		...
		pProp->dwFrameRate1000 = m_dwFrameRate1000;
        
        // MFSamplePresenter_DisplayTargetTime is set by the scheduler.
        pSample->GetUINT64(MFSamplePresenter_DisplayTargetTime, (UINT64 *)&pProp->rtDisplayTarget);

        UINT32 bRepeatFrame = 0;
        pSample->GetUINT32(MFSamplePresenter_RepeatFrame, &bRepeatFrame);
        if (bRepeatFrame)
            pProp->dwFlags |= SAMPLE_FLAG_REPEAT_FRAME;
...
STDMETHODIMP CVideoSourceMSEvr::EnableFRC(FrameRateRatio *pRatio)
{
    if (pRatio)
    {
        MFRatio mfRatio;
        mfRatio.Denominator = pRatio->Denominator;
        mfRatio.Numerator = pRatio->Numerator;
        m_scheduler.EnableFRC(&mfRatio);
    }
    else
    {
        m_scheduler.EnableFRC(NULL);
    }
    return S_OK;
}

STDMETHODIMP CVideoSourceMSEvr::SetDispFrameRate(DWORD dwFrameRate)
{	
	return m_DisplayHelper.ApplyDispFrameRateMode(dwFrameRate);
}
