...
STDMETHODIMP CDownsamplingEffect::SetParam(DWORD dwParam, DWORD dwValue)
{
    ...
    switch (dwParam)
    {
    case EFFECT_DOWNSAMPLING_PARAM_PIXEL_THRESHOLD:
        m_uPixelThreshold = dwValue;
        hr = S_OK;
        break;
    default:
        hr = E_INVALIDARG;
...
STDMETHODIMP CDownsamplingEffect::ProcessEffect(VE3DBuffer *pInput, VE3DBuffer *pOutput)
{
    ...
    if (pInput->uWidth * pInput->uHeight <= m_uPixelThreshold)
    {
        pOutput->uWidth = pInput->uWidth;
        pOutput->uHeight = pInput->uHeight;
        pOutput->pSurface = pInput->pSurface;
