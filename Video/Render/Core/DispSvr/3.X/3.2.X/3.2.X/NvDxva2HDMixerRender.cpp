...
CNvDxva2HDMixerRender::CNvDxva2HDMixerRender()
{
    ...
    // NV DXVAHD is a special path on vista/win7 to enable xvycc, perhaps we should deprecate the path.
    m_VPInitParam.bNvDXVAHD = true;
}
...
STDMETHODIMP CNvDxva2HDMixerRender::_SetDevice(IUnknown *pDevice)
{
	...
	{	
		...
		if (FAILED(hr))
        {
            DbgMsg("Creating NvDxva2HD fail");
			return hr;
        }
		...
		{
            DbgMsg("Creating CreateNvAPI fail on NvDXVA2HD");
			DestroyNvAPI();
		}
	}
