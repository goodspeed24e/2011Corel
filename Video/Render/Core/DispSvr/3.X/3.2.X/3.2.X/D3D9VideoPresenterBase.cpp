...
STDMETHODIMP CD3D9VideoPresenterBase::_SetDevice(IUnknown *pDevice)
{
	...
	m_szSrc.cx = GetRegistry(REG_BACKBUFFER_WIDTH, 0);
	m_szSrc.cy = GetRegistry(REG_BACKBUFFER_HEIGHT, 0);

	// Update the default presenter caps
	m_PresenterCaps.dwFPS = 30;
	m_PresenterCaps.dwBandWidth = 0;
	m_PresenterCaps.dwResPixels = GetRegistry(REG_DISPLAY_WIDTH, 0) * GetRegistry(REG_DISPLAY_HEIGHT, 0);
...
STDMETHODIMP CD3D9VideoPresenterBase::QueryCaps(PresenterCaps* pCaps)
{
	...
	// query driver and update m_PresenterCaps to date.
	if (SUCCEEDED(hr) && pDrvExt)
	{
		hr = pDrvExt->QueryPresenterCaps(pCaps->VideoDecodeCaps, &m_PresenterCaps);
	}
...
bool CD3D9VideoPresenterBase::IsWindowFullScreen()
{
	bool bIsFullScreen = 
		m_rcDst.left == 0 && 
		m_rcDst.top == 0 &&
		m_rcDst.right == GetRegistry(REG_DISPLAY_WIDTH, 0) &&
		m_rcDst.bottom == GetRegistry(REG_DISPLAY_HEIGHT, 0);
