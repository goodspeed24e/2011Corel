...
public:
    ...
    /// Set content frame rate, it also implicitly sets display frame rate.
    void SetFrameRate(const MFRatio& fps);
    void EnableFRC(const MFRatio *pSuggestedFPS);
    void SetClockRate(float fRate) { m_fRate = fRate; }

    const LONGLONG& LastSampleTime() const { return m_LastSampleTime; }
    const LONGLONG& FrameDuration() const { return m_frContent.PerFrameInterval; }
...
private:
    ...
    {
        MFTIME PerFrameInterval;
        LONGLONG PerFrame_1_4th;

        FrameRateInfo() : PerFrameInterval(0), PerFrame_1_4th(0) {}
        void SetRate(const MFRatio& fps);
        bool operator== (const FrameRateInfo &a) const { return a.PerFrameInterval == PerFrameInterval; }
    };
    HRESULT ProcessSamplesInQueue(LONG *plNextSleep);
    HRESULT ProcessSample(IMFSample *pSample, LONG *plNextSleep, const FrameRateInfo &fr);
    // process sample for frame rate conversion.
    HRESULT ProcessSampleFRC(IMFSample *pSample, LONG *plNextSleep, const FrameRateInfo &fr);
...
//    SchedulerCallback   *m_pCB;     // Weak reference; do not delete.
    ...
    FrameRateInfo       m_frContent;
    FrameRateInfo       m_frDisplay;
