...
{
	HRESULT hr = S_OK;
	const D3D9Plane &plane = m_pModel->GetPlane(PLANE_MAINVIDEO);
    ...
    if (ProcessVideoEffect(PLANE_MAINVIDEO, NULL, src, dst, plane.Format, &pUnk) != S_FALSE)
    {
        if (pUnk)
        {
            ...
            if (FAILED(hr))
            {
                ...
                // Bug#111355. VBlit will crash with IDirect3DSurface9 type surface on AMD GPU.
                if (PCI_VENDOR_ID_ATI == GetRegistry(REG_VENDOR_ID, 0) && pOffscreenSurface)
                {
                    D3DSURFACE_DESC desc;
                    hr = pOffscreenSurface->GetDesc(&desc);
                    if (SUCCEEDED(hr) && 
                        !pTexture &&
                        (desc.Format == PLANE_FORMAT_ARGB || desc.Format == D3DFMT_A8R8G8B8))
                        hr = pOffscreenSurface->GetContainer(IID_IDirect3DTexture9, (void**)&pTexture);
                    else
                        hr = E_FAIL;
    ...
    else
    {
        ...
        if (m_pVideoEffect3DBlt && m_pVideoEffect3DBlt->IsEffectEnabled())
        {
            if (pOffscreenSurface)
                hr = plane.pVBlt->IntermediateTextureVBlt(pOffscreenSurface, src, &pTexture);

            if (pTexture)
            {
                CComPtr<IUnknown> pOutputSurface;
                ProcessEffectRequest Request = {0};
                Request.lpSourceRect = &src;
                Request.lpTargetRect = &dst;
	...
	if (FAILED(PlaneToScreen(plane, src, dst, rcDstClip)))
		return S_FALSE;
...
{
	HRESULT hr = S_OK;
	D3D9Plane &plane = m_pModel->GetPlane(PLANE_SUBVIDEO);
	RECT src = plane.rcSrc, dst = rcDst;

	if (!plane.IsValid() || FAILED(PlaneToScreen(plane, src, dst, rcDstClip)))
		return S_FALSE;
	...
	if (FAILED(hr))
	{
		...
		if (!m_MainCRList.empty() || lumaKey.bEnable || plane.fAlpha < 1.0f || bSurface2Texture)
		{
	...
	if (pTexture)
	{
		...
		if (lumaKey.bEnable && m_pLumaKeyPS)
		{
			...
			float fConstData[4] = {
				float(lumaKey.uLower) - 0.1f, float(lumaKey.uUpper) + 0.1f,
				0, 0
			};
...
HRESULT CD3D9VideoMixer::DrawTexturePlane(PLANE_ID PlaneID, const RECT &rcDst, const RECT &rcDstClip)
{
	...
	CComPtr<IDirect3DSurface9> pOffscreenSurface;
	CComPtr<IUnknown> pUnk;
	if (PLANE_GRAPHICS == PlaneID && ProcessVideoEffect(PLANE_GRAPHICS, NULL, src, dst, plane.Format, &pUnk) != S_FALSE)
	{
		if (pUnk)
		{
			hr = pUnk->QueryInterface(IID_IDirect3DTexture9, (void **)&pTexture);
			...
			{
#ifndef _NO_USE_D3DXDLL
				hr = D3DXSaveTextureToFile(_T("C:\\spic-eff.png"), D3DXIFF_PNG, pTexture, NULL);
#endif
			}
			if (FAILED(hr))
			{
				return E_UNEXPECTED; //doesn't expect surface path for graphic planes
			}
		}
		else
		{
	...
	{
#ifndef _NO_USE_D3DXDLL
		hr = D3DXSaveTextureToFile(_T("C:\\do.png"), D3DXIFF_PNG, pTexture, NULL);
#endif
	}
	return hr;
}
