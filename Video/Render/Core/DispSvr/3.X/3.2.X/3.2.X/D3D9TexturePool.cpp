...
{
    m_GUID = DISPSVR_RESOURCE_TEXTUREPOOL;
	m_pDevice = 0;
	m_pAccelService = 0;
...
STDMETHODIMP CD3D9TexturePool::QueryInterface(REFIID riid, void **ppv)
{
	HRESULT hr = E_NOINTERFACE;

	if (riid == __uuidof(IDispSvrTexturePool))
	{
		hr = GetInterface((IDispSvrTexturePool *)this, ppv);
	}
	else
	{
...
HRESULT CD3D9TexturePool::_SetDevice(IUnknown *pObj)
{
    HRESULT hr = CD3D9PluginBase::_SetDevice(pObj);
    if (FAILED(hr))
        return hr;
	...
	m_TextureCap.clear();
	ASSERT(m_Pool.empty());
	hr = m_pDevice->GetDirect3D(&pD3D9);
	if (FAILED(hr))
		return hr;

	hr = m_pDevice->GetDeviceCaps(&deviceCaps);
	if (FAILED(hr))
		return hr;
    ...
    textureCaps.d3dFormat = D3DFMT_X8R8G8B8;
    m_TextureCap[PLANE_FORMAT_XRGB] = textureCaps;
...
HRESULT CD3D9TexturePool::_ReleaseDevice()
{
	ReclaimResources();
...
HRESULT CD3D9TexturePool::CreateTexture(CreateTextureParam *pParam, HANDLE *pTexture)
{
    ...
    DWORD dwD3DUsage = (pParam->eUsage & TEXTURE_USAGE_RESTRICTED_CONTENT) ? D3DUSAGE_RESTRICTED_CONTENT : 0;
	...
	case PLANE_FORMAT_ARGB:	//< 32 bit, A8R8G8B8
		...
		if (ePoolUsage == TEXTURE_POOL_USAGE_D3D9)
		{
			...
			if (pParam->eUsage & TEXTURE_USAGE_RENDERTARGET)
			{
				...
				hr = m_pDevice->CreateTexture(pParam->uWidth, pParam->uHeight, 1, dwD3DUsage, textureCap.d3dFormat, D3DPOOL_DEFAULT, &texture.pTextureVidMem, NULL);
			}
			else
			{
				hr = m_pDevice->CreateTexture(pParam->uWidth, pParam->uHeight, 1, dwD3DUsage, textureCap.d3dFormat, D3DPOOL_DEFAULT, &texture.pTextureVidMem, NULL);
				if (SUCCEEDED(hr))
				{
					hr = CreateSurface(pParam->uWidth, pParam->uHeight, textureCap.d3dFormat, D3DPOOL_SYSTEMMEM, false, &texture.pSurfaceSysMem, dwD3DUsage);
		...
		else
		{
			if (pParam->eUsage & TEXTURE_USAGE_RENDERTARGET)
			{
				hr = CreateSurface(pParam->uWidth, pParam->uHeight, textureCap.d3dFormat, D3DPOOL_DEFAULT, true, &texture.pSurfaceVidMem, dwD3DUsage);
			}
			else
			{
				hr = CreateSurface(pParam->uWidth, pParam->uHeight, textureCap.d3dFormat, D3DPOOL_DEFAULT, false, &texture.pSurfaceVidMem, dwD3DUsage);
				if (FAILED(hr))
					break;
	...
	case PLANE_FORMAT_P8:	//< 8 bit, I8 indexed
		...
		if (ePoolUsage == TEXTURE_POOL_USAGE_D3D9)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				hr = m_pDevice->CreateTexture(pParam->uWidth, pParam->uHeight, 1, dwD3DUsage, textureCap.d3dFormat, D3DPOOL_SYSTEMMEM, &texture.pTextureSysMem, NULL);
				if (SUCCEEDED(hr))
			...
			if (SUCCEEDED(hr))
			{
				...
				if (!textureCap.bNativeTexture)
				{
					...
					hr = m_pDevice->CreateTexture(256, 1, 1, dwD3DUsage, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &texture.pPaletteTextureVidMem, NULL);
					if (SUCCEEDED(hr))
					{
...
HRESULT CD3D9TexturePool::CreateSurface(UINT uWidth, UINT uHeight, D3DFORMAT Format, D3DPOOL Pool, bool bRenderTarget, IDirect3DSurface9** ppSurface, DWORD dwD3DUsage)
{
	HRESULT hr;
	...
	if (bRenderTarget && Pool == D3DPOOL_DEFAULT && m_pAccelService)
		hr = m_pAccelService->CreateSurface(uWidth, uHeight, 0, Format, Pool, dwD3DUsage, DXVA2_VideoProcessorRenderTarget, ppSurface, NULL);
	else if (m_pDeviceEx)
		hr = m_pDeviceEx->CreateOffscreenPlainSurfaceEx(uWidth, uHeight, Format, Pool, ppSurface, NULL, dwD3DUsage);
    else
