		...
		if (m_pKsSet)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				_FlagMap FlagMap[] =
				{
					...
					{DISPSVR_USE_STENCIL_BUFFER, IDC_USE_STECIL_BUFFER},
            ...
            BOOL bForceVMR9;
            hr = m_pKsSet->Get(PROPSET_DISPSVR, PROP_FORCE_VMR9, NULL, NULL,
                (LPVOID)(&bForceVMR9), sizeof(bForceVMR9), &dwReturn);
            if (SUCCEEDED(hr))
            {
                if (bForceVMR9)
                    CheckDlgButton(m_Dlg, IDC_FORCEVMR9, BST_CHECKED);
                else
                    CheckDlgButton(m_Dlg, IDC_FORCEVMR9, BST_UNCHECKED);
            }
		}
	}
	return S_OK;
	...
	case WM_COMMAND:
		{
			...
			switch(wParam)
			{
				...
				case IDC_USE_STECIL_BUFFER:
					{
						if (IsDlgButtonChecked(m_Dlg, IDC_USE_STECIL_BUFFER) == BST_CHECKED)
							m_DispSvrInitFlags |= DISPSVR_USE_STENCIL_BUFFER;
						else
							m_DispSvrInitFlags &= ~DISPSVR_USE_STENCIL_BUFFER;
                ...
                case IDC_FORCEVMR9:
                    {
                        BOOL bForceVMR9 = (IsDlgButtonChecked(m_Dlg, IDC_DEVICE_LOST_NOTIFY) == BST_CHECKED) ? TRUE : FALSE;

                        if (m_pKsSet)
                            hr = m_pKsSet->Set(PROPSET_DISPSVR, PROP_FORCE_VMR9, NULL, NULL, ((LPVOID)(&bForceVMR9)), sizeof(bForceVMR9));
                        return (INT_PTR)TRUE;
                    }
				default:
					bUICommand = FALSE;
