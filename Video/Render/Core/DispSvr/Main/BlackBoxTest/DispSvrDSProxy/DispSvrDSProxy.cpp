...
static BOOL g_bIsEraseBkgnd = TRUE;
...
STDMETHODIMP CDispSvrDSProxy::Set(REFGUID guidPropSet, DWORD dwPropID, LPVOID pInstanceData, DWORD cbInstanceData, LPVOID pPropData, DWORD cbPropData)
{
	if (IsEqualGUID( guidPropSet, PROPSET_DISPSVR))
	{
        ...
        else if (dwPropID == PROP_FORCE_VMR9)
        {
            if (pPropData)
            {
                m_bForceVMR9 = *((BOOL *)pPropData);
                return S_OK;
            }
            else
                return E_INVALIDARG;
...
STDMETHODIMP CDispSvrDSProxy::Get(REFGUID guidPropSet, DWORD dwPropID, LPVOID pInstanceData, DWORD cbInstanceData, LPVOID pPropData, DWORD cbPropData, DWORD *pcbReturned)
{
	if (IsEqualGUID( guidPropSet, PROPSET_DISPSVR))
	{
        ...
        else if (dwPropID == PROP_FORCE_VMR9)
        {
            if (pPropData)
            {
                *((BOOL *)pPropData) = m_bForceVMR9;
                if (pcbReturned)
                    *pcbReturned = sizeof(m_bForceVMR9);
                return S_OK;
            }
            else
                return E_INVALIDARG;
...
*/
        if (m_dwDispSvrInitFlags & DISPSVR_USE_CUSTOMIZED_OUTPUT)
            g_bIsEraseBkgnd = TRUE;
        else
            g_bIsEraseBkgnd = FALSE;
        ...
        m_pRenderEngine->SetColorKey( 0x081008);

		if (SUCCEEDED(hr))
		{
			RECT rect;
			::GetWindowRect(m_hVideo, &rect);
            //ignore return value since setdisplaywindow might return error code. e.g. Intel G45 with Cutsomized Output.
			m_pRenderEngine->SetDisplayWindow(m_hVideo, &rect);
		}
    ...
    {
        MessageBox(m_hGraphEdit, "Cannot get DispSvr instance, please register DispSvr.dll First.", "Critical Error", MB_OK);
		return E_UNEXPECTED;
    }
...
LRESULT CALLBACK CDispSvrDSProxy::VideoWndProc(HWND hwnd , UINT msg , WPARAM wp , LPARAM lp)
{
	...
	switch (msg) {
        case WM_ERASEBKGND:
             if (!g_bIsEraseBkgnd) // do not update when using D3D mode
             {
                    return true;
             }
            break;
...
HRESULT CDispSvrDSProxy::InitVideoDialog()
{
    ...
    g_BKBrush = CreateSolidBrush(COLORREF(0x081008));
...
//	m_pEvr.CoCreateInstance(CLSID_EnhancedVideoRenderer);
	...
	if ((g_osInfo.dwMajorVersion >= 6) && !m_bForceVMR9)
	{
