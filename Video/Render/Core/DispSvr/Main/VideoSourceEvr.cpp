...
public:
    ...
    HRESULT Pop(IMFSample **ppSample)
    {
        if (ppSample == NULL)
            return E_POINTER;
...
CVideoSourceEvr::~CVideoSourceEvr()
{
    ...
    m_pDispObj.Release();
    SAFE_DELETE(m_pSamplePool);
    SAFE_DELETE(m_pPresentQueue);
}
        ...
        while (hr != MF_E_NO_MORE_TYPES)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(pType->GetGUID(MF_MT_SUBTYPE, &guidSubtype)))
                {
					...
					else if (CVideoSourceDisplayHelper::IsYUVFormat(FourCC))
                    {
...
void CVideoSourceEvr::PresentLoop()
{
    ...
    while (m_eThreadStatus == eRunning)
    {
...

        {
            ...
            if (m_bFlushPresentQueue)
            {
                while (m_pPresentQueue->Size() > m_uFlushPendingPresentQueue)
                {
                    ...
                    m_vecRecycledSamples.push_back(pSample);
                    pSample.Release();
                }
                // release all surfaces that kept in DO
                if (m_uFlushPendingPresentQueue == 0)
                    m_pDispObj->ProcessMessage(m_hwndVideo, VIDEOSOURCEDO_REQUEST_RELEASE_SURFACE, 0, 0);
                ReturnRecycledSamples();
                m_bFlushPresentQueue = false;
            }
            ...
            if (SUCCEEDED(hr) && m_pClock && m_ePresenterState == PRESENTER_STATE_STARTED)
            {
                ...
                if (m_DisplayHelper.IsOverDisplayFrameRate(hnsDuration) && m_DisplayHelper.IsOverDisplayFrameRate(hnsPresent - m_hnsLastPts))
                {
        ...
        {
            if (m_DisplayHelper.IsOverDisplayFrameRate(hnsDuration) || m_DisplayHelper.IsSkipFrameForVideoSync(hnsDuration, hnsDelta))
            {
                //Skip.
                VSE_DP("Skip Frame...\n");
            }
...
HRESULT CVideoSourceEvr::ReturnSample(IMFSample *pSample)
{
    HRESULT hr = E_FAIL;
    ...
    {
        hr = m_pDispObj->ProcessMessage(m_hwndVideo, VIDEOSOURCEDO_REQUEST_QUERY_SURFACE_STATUS, 0, reinterpret_cast<LONG>(pSurface));
        if (SUCCEEDED(hr))
        {
            m_pSamplePool->ReturnSample(pSample);
        }
        pSurface->Release();
    }
...
void CVideoSourceEvr::ReturnRecycledSamples()
{
    HRESULT hr = E_FAIL;
    std::vector<IMFSample*>::iterator it = m_vecRecycledSamples.begin();

    while (it != m_vecRecycledSamples.end())
    {
        hr = ReturnSample(*it);
        if (SUCCEEDED(hr))
        {
            it = m_vecRecycledSamples.erase(it);
        }
        else
        {
            it++;
...
void CVideoSourceEvr::ClearRecycledSamples()
{
    std::vector<IMFSample*>::iterator it = m_vecRecycledSamples.begin();

    for (;it != m_vecRecycledSamples.end(); it++)
    {
        m_pSamplePool->ReturnSample(*it);
    }
    m_vecRecycledSamples.clear();
}
