...
//    MFFrameRateToAverageTimePerFrame(fps.Numerator, fps.Denominator, &AvgTimePerFrame);
	...
	AvgTimePerFrame = (Temp / 10) + ((Temp % 10) >= 5 ? 1 : 0);
...
HRESULT CEvrPresentScheduler::ProcessSample(IMFSample *pSample, LONG *plNextSleep)
{
    HRESULT hr = S_OK;
	...
	BOOL bInterlaced = MFGetAttributeUINT32(pSample, MFSampleExtension_Interlaced, FALSE);
	UINT32 uFieldSelect = MFGetAttributeUINT32(pSample, MFSamplePresenter_FieldSelect, 0);
    ...
    if (m_pClock)
    {
		...
		if (bInterlaced && uFieldSelect == 1)
			hnsPresentationTime += hnsSampleDuration/2;
        ...
        else if (hnsDelta > (3 * m_PerFrame_1_4th))
        {
            ...
            lNextSleep = (LONG)((hnsDelta - (3 * m_PerFrame_1_4th)) / 10000);
...
//        hr = m_pCB->PresentSample(pSample, hnsPresentationTime);
		...
		if (bInterlaced && uFieldSelect == 0)
		{
			pSample->SetUINT32(MFSamplePresenter_FieldSelect, ++uFieldSelect);
...
HRESULT CEvrPresentScheduler::UpdatePresentSample(IMFSample *pSample, LONGLONG llDelta)
{
	...
	CComPtr<IMFSample> pPreviousSample = m_pPresentSample;
