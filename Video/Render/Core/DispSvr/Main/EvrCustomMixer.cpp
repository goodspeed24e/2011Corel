...
static HRESULT CopySample(IMFSample *pOut, IMFSample *pIn)
{
    CComPtr<IMFMediaBuffer> pBuffer;
	CComPtr<IDirect3DDevice9> pDevice;
	CComPtr<IDirect3DSurface9> pSrc, pDst;
    HRESULT hr = pIn->GetBufferByIndex(0, &pBuffer);
    if (SUCCEEDED(hr))
    {
        CComQIPtr<IMFGetService> pGetSurface = pBuffer;
        if (pGetSurface)
            hr = pGetSurface->GetService(MR_BUFFER_SERVICE, __uuidof(IDirect3DSurface9), (void **)&pSrc);
    }
	pBuffer.Release();

    hr = pOut->GetBufferByIndex(0, &pBuffer);
    if (SUCCEEDED(hr))
    {
        CComQIPtr<IMFGetService> pGetSurface = pBuffer;
        if (pGetSurface)
            hr = pGetSurface->GetService(MR_BUFFER_SERVICE, __uuidof(IDirect3DSurface9), (void **)&pDst);
    }
	...
	if (pSrc && pDst)
	{
		...
		if (pDevice)
			pDevice->StretchRect(pSrc, NULL, pDst, NULL, D3DTEXF_NONE);
	}

	LONGLONG hns = 0;
	if (SUCCEEDED(pIn->GetSampleDuration(&hns)))
		pOut->SetSampleDuration(hns);

	if (SUCCEEDED(pIn->GetSampleTime(&hns)))
		pOut->SetSampleTime(hns);

	pOut->SetUINT32(MFSampleExtension_Interlaced, MFGetAttributeUINT32(pIn, MFSampleExtension_Interlaced, FALSE));
	pOut->SetUINT32(MFSampleExtension_Discontinuity, MFGetAttributeUINT32(pIn, MFSampleExtension_Discontinuity, FALSE));
	pOut->SetUINT32(MFSampleExtension_BottomFieldFirst, MFGetAttributeUINT32(pIn, MFSampleExtension_BottomFieldFirst, FALSE));
	pOut->SetUINT32(MFSampleExtension_RepeatFirstField, MFGetAttributeUINT32(pIn, MFSampleExtension_RepeatFirstField, FALSE));
...
STDMETHODIMP CEvrCustomMixer::GetInputCurrentType(DWORD dwInputStreamID, IMFMediaType **ppType)
{
	...
	CM_DP("GetInputCurrentType(dwInputStreamID=%d)", dwInputStreamID);
	hr = VerifyInputStreamID(dwInputStreamID);
	if (ppType == 0)
		return E_INVALIDARG;
...
STDMETHODIMP CEvrCustomMixer::GetOutputCurrentType(DWORD dwOutputStreamID, IMFMediaType **ppType)
{
	...
	CM_DP("GetOutputCurrentType(dwOutputStreamID=%d)", dwOutputStreamID);
	hr = VerifyOutputStreamID(dwOutputStreamID);
	if (ppType == 0)
		return E_INVALIDARG;
...
STDMETHODIMP CEvrCustomMixer::GetInputStatus(DWORD dwInputStreamID, DWORD *pdwFlags)
{
	...
	CM_DP("GetInputStatus(dwInputStreamID=%d, pdwFlags=%x[%d])", dwInputStreamID, pdwFlags, pdwFlags ? *pdwFlags : 0);
	if (pdwFlags == 0)
		return E_INVALIDARG;
...
{
	CM_DP("GetOutputStatus(pdwFlags=%x[%d])", pdwFlags, pdwFlags ? *pdwFlags : 0);
	if (pdwFlags == 0)
		return E_INVALIDARG;
...
{
	CM_DP("ProcessEvent(dwInputStreamID=%d, IMFMediaEvent pEvent=0x%x)", dwInputStreamID, pEvent);
	return E_NOTIMPL;
}
	...
	DWORD *pdwStatus)
{
	...
	for (; i < cOutputBufferCount && i < m_nStreams; i++)
	{
		if (m_pSamples[i])
		{
			...
			{
				CopySample(pOutputSamples[i].pSample, m_pSamples[i]);
			}
			else
			{
...
STDMETHODIMP CEvrCustomMixer::InitServicePointers(IMFTopologyServiceLookup* pLookup)
{
	...
	MF_SERVICE_LOOKUP_TYPE eType = MF_SERVICE_LOOKUP_GLOBAL;	// type is currently ignored
	...
	*ppType = 0;
	...
	if (SUCCEEDED(hr))
	{
		...
		*ppType = pmt;
		...
		MFVideoArea sMinArea;
		if (SUCCEEDED(m_pInputTypes[0]->GetBlob(MF_MT_MINIMUM_DISPLAY_APERTURE, (UINT8 *)&sMinArea, sizeof(sMinArea), NULL)))
		{
			ASSERT(sMinArea.OffsetX.value == 0 && sMinArea.OffsetY.value == 0);
			uWidth = sMinArea.Area.cx;
			uHeight = sMinArea.Area.cy;
		}
