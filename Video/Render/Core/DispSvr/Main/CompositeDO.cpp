...
STDMETHODIMP CVideoRootDO::NonDelegatingQueryInterface(REFIID iid, void **ppv)
{
	...
	{
		return m_pVideoMixer->QueryInterface(iid, ppv);
	}
	return IParentDisplayObjectImpl::NonDelegatingQueryInterface(iid, ppv);
}

STDMETHODIMP CVideoRootDO::Terminate()
{
	IParentDisplayObjectImpl::Terminate();
	m_pVideoMixer.Release();
	return S_OK;
}

STDMETHODIMP CVideoRootDO::Initialize(IDisplayRenderEngine* pRenderEngine)
{
	HRESULT hr = pRenderEngine->QueryInterface(__uuidof(IDispSvrVideoMixer), (void **)&m_pVideoMixer);

	if (FAILED(hr))
		return hr;

	return IParentDisplayObjectImpl::Initialize(pRenderEngine);
}
...
STDMETHODIMP CVideoRootDO::EndDeviceLoss(IUnknown* pDevice)
{
	HRESULT hr = IParentDisplayObjectImpl::EndDeviceLoss(pDevice);
	m_pVideoMixer = 0;
	return CResourceManager::GetInstance()->GetInterface(DISPSVR_RESOURCE_VIDEOMIXER, __uuidof(IDispSvrVideoMixer), (void **)&m_pVideoMixer);
}