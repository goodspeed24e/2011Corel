...
#define _IVI_DUMMYCALL_H
	...
	#define iviTR_DUMMY_CALL_3(Num)														\
		...
		{																				\
			...
			if (::RegOpenKeyEx(HKEY_LOCAL_MACHINE, szKey##Num, 0, KEY_QUERY_VALUE, &hKey##Num) == ERROR_SUCCESS) \
			{																			\
				if (::RegOpenKeyEx(hKey##Num, szSubKey##Num, 0, KEY_QUERY_VALUE, &hSubKey##Num) == ERROR_SUCCESS) \
				{																		\
					bError##Num = FALSE;												\
				}																		\
			}																			\
